...
Last updated: 2026-07-11 (Section 28 — Manifest Cross-Check: switched to reading Auto-Sync's manifest as a Google Sheet, after the CSV it replaced was confirmed to be getting silently reformatted by an external spreadsheet app)
...
**Pipeline side** (`process.py`): `_build_manifest_cross_check()` (near `_STREAM_TABLES`) fetches the manifest via `drive_connector.fetch_download_manifest()`, maps each manifest file-name slot to a pipeline stream id (`_MANIFEST_SLOT_TO_STREAM`), and for the most recent `MANIFEST_CROSS_CHECK_WINDOW_DAYS` (14) data-dates compares Auto-Sync's Verified/Missing claim against real ingestion — exact per-date row presence for streams with daily-granularity tables (same date sets `stream_gaps` already uses), falling back to a `date <= watermark` approximation for streams that only track a `*_last_date` cutoff. Result is written into `pipeline_run_log.json` as `manifest_cross_check` (`null` if the fetch failed that run — the dashboard must render "unavailable," never "0 discrepancies," in that case).
