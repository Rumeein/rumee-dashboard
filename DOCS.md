...
Jaiswal asked for a way to see, at a glance, which of the ~85 Product Master items already have a BOM while he goes through them one at a time (dashboard memory active.md item #64). `renderPmCatalog()` gained a per-variation "✓ BOM" / "No BOM" chip (looked up from `window._matBomsFinal[doc.sku_id]`, the same dict item #64's BOM Output-Type=Final work populates) and `loadProductsTab()` gained a parallel, non-blocking `loadBoms()` call so the marker fills in once BOM data resolves — it does not delay or block the Product Master catalog's own (public, unauthenticated) load. `_pmData` itself is never touched by this; `pmWrite` is not called anywhere in this change. `rumee_boms` requires sign-in (unlike `product_master`), so an anonymous viewer sees the catalog immediately with every item defaulting to "No BOM" until/unless BOM data actually resolves — the stats-bar counter (`— BOM` before it resolves, `N / M have a BOM` after) makes this "unknown vs confirmed zero" distinction explicit rather than silently claiming a false zero.
