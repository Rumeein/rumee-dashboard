...
**Known upstream fragility (not a dashboard bug, root cause NOT yet confirmed):** `download_manifest.csv`'s date format was observed to flip between two fetches 18 minutes apart on 2026-07-11 — all 690 existing rows rewritten from quoted `YYYY-MM-DD` to unquoted `MM-DD-YYYY` (Drive `modifiedTime` 20:01:49Z → 20:19:21Z). **Correction to an earlier version of this note:** this is NOT confirmed to be a code bug in Auto-Sync's writer — a follow-up read of the current `background.js` found both write paths (`verifyAndLogManifest()`, `rebuildManifestHistory()`) always quote values and always source dates from `istToday()`/`istAddDays()` (ist-time.js), which are hardcoded to `.toISOString().slice(0,10)` — no code path in the current source produces unquoted `MM-DD-YYYY`. More likely causes: a stale/different extension build actually running vs. what's in that repo, or something external re-saving the CSV (Excel and some Drive-sync clients silently reformat date-looking columns to system locale on open/save). A handoff prompt with this exact finding was given to Jaiswal to investigate in a separate `rumee-auto-sync` session — not yet resolved as of this writing. `drive_connector._normalize_manifest_date()` accepts either format defensively regardless of cause, so a future flip degrades to "row skipped" instead of silently miscomparing dates.
