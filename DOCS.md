...
**Agreed fix (design confirmed with Jaiswal, not yet built):** store which lot a piece-return created on that specific `returns[]` entry (now possible — `postStockMovement` returns `lotsTouched`, see above). Editing that return later targets THAT SPECIFIC lot directly (adjust its `qty_remaining`/cost), not generic FIFO. If that lot no longer has enough left to cover a downward correction (already partly sold or consumed elsewhere since), **block the edit and explain why** rather than silently drawing from a different lot — Jaiswal's explicit rejection of a "fall back to FIFO for the shortfall" design ("why do you want to callback on FIFO again which is incorrect"), matching the same "flag it, don't approximate" rule used for the naming-mismatch case above.

### Confirmed architectural gap — platform sales don't use FIFO at all (2026-08-06, item #131, discovery only)

Everything documented in this section (FIFO lots, landed cost, Correct Lot Cost, `lots_given` traceability) applies to stock movements originating in `index.html` — purchases, Job Worker consumption/returns, QC, adjustments, merges. **Platform sales are a completely separate path and do not participate in any of it.** Confirmed by reading `firestore_connector.py`'s `apply_stock_movements()` (called by `process.py`'s `_process_sale_stock_decrement`, the pipeline's order→stock-decrement step): it only ever writes to `rumee_materials` (`current_stock`/`current_avg_cost`) and `rumee_stock_ledger` — it **never touches `rumee_stock_lots`**, and for an `'out'` (sale) movement it only decrements `stock`; `avg_cost` is not reassigned on `'out'` at all (only on `'in'`). No FIFO lot depletion, no per-sale cost derivation, nothing. Practical consequence: for any finished-goods material replenished via Job Worker returns (which DO create/deplete real lots) and also sold via platform orders (which don't touch lots at all), `rumee_stock_lots`' `qty_remaining` totals will drift increasingly stale vs. the real `current_stock` over time. Not quantified — how many materials are affected and how stale the drift already is is unknown. Bringing real FIFO costing to the sales path is a separate, larger initiative than anything else in this section (Python side — `process.py`/`firestore_connector.py`, not `index.html`) and needs its own dedicated session.
