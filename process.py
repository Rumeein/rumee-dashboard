...
def save_db(db, path):
    ...
    table_schemas = {
        ...
        'me_skus':          ['sku_id', 'name', 'type', 'orders', 'total_orders', 'delivered', 'rto',
...
def generate_insights(db):
    ...
    DISCONTINUED 2026-08-09 (Jaiswal, dashboard memory active.md item #136):
    not being used, and the collection was found growing unbounded --
    insight_exists_today() only checked whether an insight was created
    TODAY, not whether an unresolved one already existed at all, so a
    persistent condition wrote a brand-new duplicate insight every single
    day forever with nothing ever pruning the old ones. Paused here rather
    than deleted so it's a one-line revert if this gets rebuilt later --
    remove the guard below to resume. The 5 stock/return-rate rules were
    also found to be redundant with data already visible directly in the
    Products tab / SKU Performance tables; the 4 claims rules + the
    Flipkart CTR-no-conversion rule were the only genuinely unique signal
    if this does get rebuilt.
    """
    print("  generate_insights: DISCONTINUED (dashboard memory item #136) -- skipping")
    return
    try:
